...
vc-controller-manager-image:
	docker buildx build -t "${IMAGE_PREFIX}/vc-controller-manager:$(TAG)" . -f ./installer/dockerfile/controller-manager/Dockerfile --output=type=${BUILDX_OUTPUT_TYPE} --platform ${DOCKER_PLATFORMS} --build-arg APK_MIRROR=${APK_MIRROR} --build-arg OPEN_EULER_IMAGE_TAG=${OPEN_EULER_IMAGE_TAG}

vc-scheduler-image:
	docker buildx build -t "${IMAGE_PREFIX}/vc-scheduler:$(TAG)" . -f ./installer/dockerfile/scheduler/Dockerfile --output=type=${BUILDX_OUTPUT_TYPE} --platform ${DOCKER_PLATFORMS} --build-arg APK_MIRROR=${APK_MIRROR} --build-arg OPEN_EULER_IMAGE_TAG=${OPEN_EULER_IMAGE_TAG}

vc-webhook-manager-image:
	docker buildx build -t "${IMAGE_PREFIX}/vc-webhook-manager:$(TAG)" . -f ./installer/dockerfile/webhook-manager/Dockerfile --output=type=${BUILDX_OUTPUT_TYPE} --platform ${DOCKER_PLATFORMS} --build-arg APK_MIRROR=${APK_MIRROR} --build-arg OPEN_EULER_IMAGE_TAG=${OPEN_EULER_IMAGE_TAG}
