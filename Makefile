...
update-development-yaml:
	...
	cp installer/volcano-latest.yaml installer/volcano-development.yaml
	sed -i 's/PullPolicy: IfNotPresent/PullPolicy: Always/g' installer/volcano-latest.yaml
	sed -i 's/PullPolicy: "IfNotPresent"/PullPolicy: "Always"/g' installer/volcano-latest.yaml
	cp installer/volcano-latest.yaml installer/volcano-latest-arm64.yaml
	sed -r -i 's#(.*)image:([^:]*):(.*)#\1image:\2-arm64:\3#' installer/volcano-latest-arm64.yaml
