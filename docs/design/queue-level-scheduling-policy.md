...
An example configuration of a job-level scheduling policy is shown below:
...
metadata:
  name: test-job
  annotations:
    scheduling.volcano.sh/scheduler-policy: SchedulerPolicy-example-2
spec:
  minAvailable: 3
...
## 1.4、Add the PluginRegistry and SchedulerPolicy
...
## 1.5、Add a new map in the session.
...
## 1.6、Add Actions in the session.
...
## 2.2、Execution of extension points
