...
2. Add the `net-topology` plugin in the scheduler configuration to implement these policies.

```yaml
- plugins:
   - name: net-topology
     arguments:
       net-topology.type: static # static means it will use the node's labels to aware network topology
       net-topology.keys: rack,switch,idc # required when type is static
