...
type PodGroupSpec struct {
    ...
    // TaskNetworkTopology defines the NetworkTopology config for each task, this field works in conjunction with network topology feature and hyperNode CRD.
	  // +optional
	  TaskNetworkTopology map[string]*NetworkTopologySpec
}
...
**Impact on TaskOrderFn:**

If a task has network topology set, then the task will have a higher priority than if it is sorted by priority and by uid.

network topology hard mode > priority > uid
