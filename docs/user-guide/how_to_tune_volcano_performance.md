...
1.  **Scenario-Dependent Performance**: Volcano's performance is highly dependent on the scheduling scenario. **When Gang Scheduling is enabled, its performance far exceeds that of other schedulers**, highlighting its core advantages in batch processing and AI scenarios. In general scenarios without Gang Scheduling, its throughput is slightly lower than simpler schedulers due to the overhead of its job management features designed for complex scenarios.
...
4.  **API-Server Contention between CREATE/SCHEDULE**: In high-concurrency scenarios, concurrent write operations to the K8s API-Server/ETCD by the Controller (CREATE) and Scheduler (SCHEDULE) trigger numerous optimistic lock conflicts (`409 Conflict`), leading to a macroscopic phenomenon of "stair-step stalls" where they appear to execute alternately. Meanwhile, blindly increasing the `CREATE` rate may actually prolong the overall scheduling time.
...
- When Gang Scheduling was not enabled, we observed unique performance characteristics in Volcano:
  ...
  1.  **Throughput and Stair-Step Stalls**: Volcano's efficiency was slightly lower than other schedulers, and the `CREATED` and `SCHEDULED` curves exhibited a distinct stair-step pattern, with alternating periods of "surges" and "stalls."

    ![CREATE/SCHEDULE Stall Phenomenon](../images/performance-tuning/stair-step-stall-phenomenon.png)

  2.  **Large-Scale Pod Creation Failures**: When the number of pods within a single job was very large (e.g., 20 Jobs × 500 Pods), fewer than 2,000 pods were successfully created under the default configuration.

    ![Pod Creation Failure Phenomenon](../images/performance-tuning/pod-creation-failure-phenomenon.png)
...
*   **Observation**: The `CREATE` and `SCHEDULE` curves almost completely overlapped, and the overall slope (throughput) was the slowest among the four benchmark groups. YuniKorn's throughput was higher than Kueue's and Volcano's, primarily due to the extra overhead from job management features (YuniKorn does not provide job management). The results were the same in both low and high-resource environments.
...
Similar to the PodGroup stage, if multiple components (like the Controller and Scheduler) update a Pod simultaneously, the operation can fail with a `409 Conflict` error due to a `resourceVersion` mismatch, requiring a retry.
...
We observed that the stair-step stalls for `CREATE` (performed by the Controller) and `SCHEDULE` (performed by the Scheduler) never occurred simultaneously. We also saw a large number (over 3000) of `409 Conflict` errors in the `kube-apiserver` logs. Further investigation revealed that **the root cause is contention from two independent components concurrently writing to a shared resource, the K8s API-Server/ETCD. These concurrent writes trigger optimistic lock conflicts**, leading to request retries and queuing, which manifests macroscopically as alternating execution.
...
**The real contention occurs at the K8s API-Server level**. After completing their respective operations, both components must submit their results (creating a Pod / updating a Pod's status) to the **K8s API-Server**, which then handles writing to etcd. When both components operate on the same Pod object simultaneously, K8s's optimistic locking mechanism is triggered. One component's write request will fail with a `409 Conflict` error due to a `resourceVersion` mismatch, forcing a retry. The large volume of concurrent requests, conflicts, and retries ultimately manifests as an alternating execution pattern where one works while the other waits.
...
The Controller processes jobs one at a time per worker. A worker goroutine waits for all pods within one job to be created before moving on to the next. During this process, if some pods in a job are blocked, it has an amplifying effect, blocking the entire goroutine. Furthermore, resource updates for a series of pods belonging to the same job will trigger updates to the same PodGroup or even the Job object, causing `409 Conflict` errors to appear consecutively for a period, leading to the "stall" phenomenon.
...
- The kube-apiserver returned a large number (over 3000) of `409 Conflict` errors to the client. This directly proves that the Controller and Scheduler were performing concurrent read/write operations on the same Pod objects, causing optimistic lock conflicts. Further log analysis is planned to pinpoint the exact stage where this contention occurs. A typical error log is shown below:
    ```
    Event Type: default_vc-controller-manager/v0.0.0 (linux/amd64) kubernetes/$Format_create_pods
    Occurrences: 1575
    Event Characteristics:
        namespace: default
        userAgent: vc-controller-manager/v0.0.0 (linux/amd64) kubernetes/$Format
        verb: create
        resource: pods
        code: 409
    ...
    Event Type: default_vc-controller-manager/v0.0.0 (linux/amd64) kubernetes/$Format_update_jobs/status
    Occurrences: 705
    Event Characteristics:
        namespace: default
        userAgent: vc-controller-manager/v0.0.0 (linux/amd64) kubernetes/$Format
    ...
    Event Type: default_vc-controller-manager/v0.0.0 (linux/amd64) kubernetes/$Format_create_podgroups
    Occurrences: 623
    Event Characteristics:
        namespace: default
        userAgent: vc-controller-manager/v0.0.0 (linux/amd64) kubernetes/$Format
    ...
    Event Type: default_vc-scheduler/v0.0.0 (linux/amd64) kubernetes/$Format_update_podgroups
    Occurrences: 477
    Event Characteristics:
        namespace: default
        userAgent: vc-scheduler/v0.0.0 (linux/amd64) kubernetes/$Format
        verb: update
        resource: podgroups
        code: 409
        reason: Conflict
