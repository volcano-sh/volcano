...
type CommandLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Command, err error)
...
// List lists all Commands in the indexer.
func (s *commandLister) List(selector labels.Selector) (ret []*v1.Command, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Command))
	})
	return ret, err
}
...
type CommandNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Command, error)
...
// List lists all Commands in the indexer for a given namespace.
func (s commandNamespaceLister) List(selector labels.Selector) (ret []*v1.Command, err error) {
...
// Get retrieves the Command from the indexer for a given namespace and name.
func (s commandNamespaceLister) Get(name string) (*v1.Command, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("command"), name)
	}
	return obj.(*v1.Command), nil
}
