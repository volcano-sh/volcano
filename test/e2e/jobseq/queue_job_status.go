...
var _ = Describe("Queue Job Status Transition", func() {
	...
	It("Transform from running to unknown after a member is gone should succeed", func() {
		...
		pg, err := ctx.Vcclient.SchedulingV1beta1().PodGroups(ctx.Namespace).Create(context.TODO(), &v1beta1.PodGroup{
		...
		for i := 0; i < 2; i++ {
			pods = append(pods, e2eutil.CreatePod(ctx, e2eutil.PodSpec{
		...
		err = e2eutil.WaitPodGroupPhase(ctx, pg, v1beta1.PodGroupRunning)
		Expect(err).NotTo(HaveOccurred())

		By("Force deleting one member so it is gone rather than Releasing")
		zero := int64(0)
		err = ctx.Kubeclient.CoreV1().Pods(ctx.Namespace).Delete(context.TODO(), pods[0].Name, metav1.DeleteOptions{
			GracePeriodSeconds: &zero,
		})
		Expect(err).NotTo(HaveOccurred())
		err = e2eutil.WaitPodGone(ctx, pods[0].Name, ctx.Namespace)
		Expect(err).NotTo(HaveOccurred())

		By("Verifying the PodGroup becomes Unknown because a required member is missing")
		err = e2eutil.WaitPodGroupPhase(ctx, pg, v1beta1.PodGroupUnknown)
