...
var _ = Describe("Unschedulable Job Cache", func() {
	...
	Describe("Generic event hints and cache behavior", func() {
		It("wakes a multi-replica Job when one rejected task is helped by a Node update", func() {
			...
			wakeupLabels["resource"] = string(fwk.Node)
		...
		It("does not affect Jobs that can be scheduled normally", func() {
			...
			skipLabels := schedulerJobMetricLabels(blockedJob, map[string]string{"stage": "allocate"})
			skipBaseline := schedulerMetricValue(ctx, skipMetricName, skipLabels)
		...
		It("does not suppress preemption for a cached high-priority Job", func() {
			...
			By("filling the only CPU slot with a low-priority Job")
			...
			skipLabels := schedulerJobMetricLabels(highJob, map[string]string{"stage": "allocate"})
			skipBaseline := schedulerMetricValue(ctx, skipMetricName, skipLabels)
			Expect(e2eutil.WaitJobUnschedulable(ctx, highJob)).To(Succeed())
			waitForMetricIncrease(ctx, skipMetricName, skipLabels, skipBaseline)
			expectJobUnbound(ctx, highJob)

			By("making the low-priority Pod preemptable while the high-priority Job remains cached")
			lowTasks := e2eutil.GetTasksOfJob(ctx, lowJob)
			Expect(lowTasks).To(HaveLen(1))
			Expect(retry.RetryOnConflict(retry.DefaultBackoff, func() error {
				pod, err := ctx.Kubeclient.CoreV1().Pods(ctx.Namespace).Get(
					context.TODO(), lowTasks[0].Name, metav1.GetOptions{})
				if err != nil {
					return err
				}
				pod.Labels[schedulingv1beta1.PodPreemptable] = "true"
				_, err = ctx.Kubeclient.CoreV1().Pods(ctx.Namespace).Update(
		...
		It("retries a cached Job after the watchdog duration expires", func() {
			...
			expirationLabels := schedulerJobMetricLabels(job, nil)
			skipLabels := schedulerJobMetricLabels(job, map[string]string{"stage": "allocate"})
...
func runTaskPodReleaseWakeupCase(pluginName, queueName, jobName string) {
	...
	skipLabels := schedulerJobMetricLabels(job, map[string]string{"stage": "allocate"})
	wakeupLabels := schedulerJobMetricLabels(job, map[string]string{"resource": string(fwk.Pod)})
	skipBaseline := schedulerMetricValue(ctx, skipMetricName, skipLabels)
	wakeupBaseline := schedulerMetricValue(ctx, wakeupMetricName, wakeupLabels)
...
func waitForJobPodGroupPhase(ctx *e2eutil.TestContext, job *batchv1alpha1.Job, phase schedulingv1beta1.PodGroupPhase) {
	Eventually(func() schedulingv1beta1.PodGroupPhase {
		podGroup, err := ctx.Vcclient.SchedulingV1beta1().PodGroups(job.Namespace).Get(
			context.TODO(), schedulerJobName(job), metav1.GetOptions{})
		if err != nil {
			return ""
		}
...
func schedulerJobMetricLabels(job *batchv1alpha1.Job, additional map[string]string) map[string]string {
