...
func CreateQueueWithQueueSpec(ctx *TestContext, queueSpec *QueueSpec) {
	_, err := ctx.Vcclient.SchedulingV1().Queues().Get(context.TODO(), queueSpec.Name, metav1.GetOptions{})
	if err != nil {
		...
		_, err := ctx.Vcclient.SchedulingV1().Queues().Create(context.TODO(), queue, metav1.CreateOptions{})
...
func CreateQueue(ctx *TestContext, q string) {
	_, err := ctx.Vcclient.SchedulingV1().Queues().Get(context.TODO(), q, metav1.GetOptions{})
	if err != nil {
		_, err := ctx.Vcclient.SchedulingV1().Queues().Create(context.TODO(), &vcschedulingv1.Queue{
			ObjectMeta: metav1.ObjectMeta{
				Name: q,
			},
			Spec: vcschedulingv1.QueueSpec{
...
func DeleteQueue(ctx *TestContext, q string) {
	...
	queue, err := ctx.Vcclient.SchedulingV1().Queues().Get(context.TODO(), q, metav1.GetOptions{})
	Expect(err).NotTo(HaveOccurred(), "failed to get queue %s", q)

	queue.Status.State = vcschedulingv1.QueueStateClosed
	_, err = ctx.Vcclient.SchedulingV1().Queues().UpdateStatus(context.TODO(), queue, metav1.UpdateOptions{})
	...
	err = ctx.Vcclient.SchedulingV1().Queues().Delete(context.TODO(), q,
...
func SetQueueReclaimable(ctx *TestContext, queues []string, reclaimable bool) {
	...
	for _, q := range queues {
		...
		_, err = ctx.Vcclient.SchedulingV1().Queues().Update(context.TODO(), queue, metav1.UpdateOptions{})
...
func queueClosed(ctx *TestContext, name string) wait.ConditionFunc {
	return func() (bool, error) {
		queue, err := ctx.Vcclient.SchedulingV1().Queues().Get(context.TODO(), name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}

		if queue.Status.State != vcschedulingv1.QueueStateClosed {
			return false, nil
		}
