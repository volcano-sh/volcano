...
func TestAddPod(t *testing.T) {
			...
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						"volcano.sh/task1-bunch-id": "0",
						"volcano.sh/partition-id":   "volcano.sh_task1-bunch-id_0",
					},
					Annotations: map[string]string{vcbatchv1.JobNameKey: "job1",
						vcbatchv1.JobVersion:  "0",
						vcbatchv1.TaskSpecKey: "task1"},
				...
				BunchGroup: map[string]*BunchInfo{
					"task1": &BunchInfo{
						...
						MatchPolicy: []*v1beta1.MatchPolicySpec{
							&v1beta1.MatchPolicySpec{
								LabelKey: "volcano.sh/task1-bunch-id",
							},
						},
	...
	for i, testcase := range testCases {
		...
		if testcase.jobinfo.BunchGroup != nil {
			if _, ok := testcase.jobinfo.BunchGroup["task1"].PartitionGroup["volcano.sh_task1-bunch-id_0"][testcase.pod.Name]; ok != testcase.ExpectValue {
				t.Errorf("case %d (%s): expected: %v, got %v ", i, testcase.Name, testcase.ExpectValue, ok)
			}
		}
...
func TestDeletePod(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Name: "deletePodForPartitionGroup",
			pod: &v1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					UID:       types.UID(fmt.Sprintf("%v-%v", namespace, name)),
					Name:      name,
					Namespace: namespace,
					Labels: map[string]string{
						"volcano.sh/task1-bunch-id": "volcano.sh_task1-bunch-id_0",
					},
					Annotations: map[string]string{vcbatchv1.JobNameKey: "job1",
						vcbatchv1.JobVersion:  "0",
						vcbatchv1.TaskSpecKey: "task1"},
	...
	for i, testcase := range testCases {
		...
		if testcase.jobinfo.BunchGroup != nil {
			testcase.jobinfo.BunchGroup["task1"].PartitionGroup["volcano.sh_task1-bunch-id_0"] = make(map[string]*v1.Pod)
			testcase.jobinfo.BunchGroup["task1"].PartitionGroup["volcano.sh_task1-bunch-id_0"][testcase.pod.Name] = testcase.pod
		}
...
func TestClone(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			jobinfo: JobInfo{
				Name:       "testjobInfo",
				Pods:       make(map[string]map[string]*v1.Pod),
				BunchGroup: make(map[string]*BunchInfo),
			},
		},
	}
	...
	for i, testcase := range testCases {
		...
		testcase.jobinfo.BunchGroup["task1"] = bunchInfo
