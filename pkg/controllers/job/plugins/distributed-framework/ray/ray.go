...
func (rp *rayPlugin) createServiceIfNotExist(job *batch.Job) error {
	...
	if _, err := rp.Clientset.KubeClients.CoreV1().Services(job.Namespace).Get(context.TODO(), job.Name, metav1.GetOptions{}); err != nil {
		if !apierrors.IsNotFound(err) {
