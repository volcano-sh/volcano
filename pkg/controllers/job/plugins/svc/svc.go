...
func (sp *servicePlugin) OnPodCreate(pod *v1.Pod, job *vcbatchv1.Job) error {
...
func (sp *servicePlugin) OnJobAdd(job *vcbatchv1.Job) error {
...
func (sp *servicePlugin) OnJobDelete(job *vcbatchv1.Job) error {
	if job.Status.ControlledResources["plugin-"+sp.Name()] != sp.Name() {
		return nil
	}
...
func (sp *servicePlugin) OnJobUpdate(job *vcbatchv1.Job) error {
	hostFile := GenerateHosts(job)
...
func (sp *servicePlugin) mountConfigmap(pod *v1.Pod, job *vcbatchv1.Job) {
...
func (sp *servicePlugin) createServiceIfNotExist(job *vcbatchv1.Job) error {
		...
		svc := &v1.Service{
			...
			Spec: v1.ServiceSpec{
				...
				Selector: map[string]string{
					vcbatchv1.JobNameKey:      job.Name,
					vcbatchv1.JobNamespaceKey: job.Namespace,
				},
...
func (sp *servicePlugin) createNetworkPolicyIfNotExist(job *vcbatchv1.Job) error {
...
func (sp *servicePlugin) cmName(job *vcbatchv1.Job) string {
	return fmt.Sprintf("%s-%s", job.Name, sp.Name())
}

// GenerateHosts generates hostnames per task.
func GenerateHosts(job *vcbatchv1.Job) map[string]string {
