...
func TestAbortedState_Execute(t *testing.T) {
				...
				Job: &vcbatchv1.Job{
					...
					Status: vcbatchv1.JobStatus{
						State: vcbatchv1.JobState{
							Phase: vcbatchv1.Aborted,
						},
					},
				},
			},
			Action:      vcbusv1.ResumeJobAction,
			ExpectedVal: nil,
		},
		{
	...
	for _, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			_, err := fakecontroller.vcClient.BatchV1().Jobs(namespace).Create(context.TODO(), testcase.JobInfo.Job, metav1.CreateOptions{})
			if err != nil {
			...
			if testcase.Action == vcbusv1.ResumeJobAction {
				...
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Restarting {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.Action != vcbusv1.ResumeJobAction {
				...
				if testcase.JobInfo.Job.Status.Pending == 0 && testcase.JobInfo.Job.Status.Running == 0 && testcase.JobInfo.Job.Status.Terminating == 0 {
					if jobInfo.Job.Status.State.Phase != vcbatchv1.Aborted {
						t.Error("Phase Should be aborted")
					}
				} else {
					if jobInfo.Job.Status.State.Phase != vcbatchv1.Aborting {
						t.Error("Phase Should be aborted")
					}
				}
			}
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.JobInfo.Job.Status.Running == 0 && testcase.JobInfo.Job.Status.Pending == 0 && testcase.JobInfo.Job.Status.Terminating == 0 {
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Completed {
					fmt.Println(jobInfo.Job.Status.State.Phase)
					t.Errorf("Expected Phase to be Completed State in test case: %d", i)
				}
			} else {
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Completing {
...
func TestPendingState_Execute(t *testing.T) {
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.Action == vcbusv1.RestartJobAction {
				// always jump to restarting firstly
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Restarting {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Restarting, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == vcbusv1.AbortJobAction {
				// always jump to aborting firstly
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Aborting {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Aborting, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == vcbusv1.TerminateJobAction {
				// always jump to completing firstly
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Terminating {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Terminating, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == vcbusv1.CompleteJobAction {
				// always jump to completing firstly
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Completing {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Completing, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == vcbusv1.EnqueueAction {
				if jobInfo.Job.Spec.MinAvailable <= jobInfo.Job.Status.Running+jobInfo.Job.Status.Succeeded+jobInfo.Job.Status.Failed {
					if jobInfo.Job.Status.State.Phase != vcbatchv1.Running {
						t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Running, jobInfo.Job.Status.State.Phase, i)
					}
				}
			} else {
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Pending {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Pending, jobInfo.Job.Status.State.Phase, i)
				}
			}
		})
	}
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.JobInfo.Job.Spec.MaxRetry <= testcase.JobInfo.Job.Status.RetryCount {
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Failed {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Failed, jobInfo.Job.Status.State.Phase, i)
				}
			} else {
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Pending {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Pending, jobInfo.Job.Status.State.Phase, i)
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.Action == vcbusv1.RestartJobAction {
				...
				if total == testcase.JobInfo.Job.Status.Succeeded+testcase.JobInfo.Job.Status.Failed {
					if jobInfo.Job.Status.State.Phase != vcbatchv1.Completed {
						t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Completed, jobInfo.Job.Status.State.Phase, i)
					}
				} else {
					if jobInfo.Job.Status.State.Phase != vcbatchv1.Running {
						t.Errorf("Expected Job phase to %s, but got %s in case %d", vcbatchv1.Running, jobInfo.Job.Status.State.Phase, i)
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.JobInfo.Job.Status.Running == 0 && testcase.JobInfo.Job.Status.Pending == 0 && testcase.JobInfo.Job.Status.Terminating == 0 {
				...
				if jobInfo.Job.Status.State.Phase != vcbatchv1.Terminated {
					fmt.Println(jobInfo.Job.Status.State.Phase)
