...
func createJobPod(job *batch.Job, template *v1.PodTemplateSpec, ix int, jobForwarding bool, pg *schedulingv2.PodGroup, ts *batch.TaskSpec) *v1.Pod {
	...
	if pg != nil && ts.PartitionPolicy != nil && pg.Spec.BunchPolicy != nil {
		partitionSize := ts.PartitionPolicy.PartitionSize
		taskName := ts.Name
		for _, bunchPolicy := range pg.Spec.BunchPolicy {
			// The taskName in vcjob corresponds one-to-one with the name in bunchPolicy.
			if bunchPolicy.Name != taskName {
				continue
			}
			matchPolicys := bunchPolicy.MatchPolicy
			if matchPolicys != nil {
				for _, matchPolicy := range matchPolicys {
					labelKey := matchPolicy.LabelKey
					id := strconv.FormatInt(int64(int32(ix)/partitionSize), 10)
...
func applyPolicies(job *batch.Job, req *apis.Request) (delayAct *delayAction) {
	delayAct = &delayAction{
		...
		podName:        req.PodName,
		podUID:         req.PodUID,
		partitionGroup: req.PartitionGroup,
...
func GetStateAction(delayAct *delayAction) state.Action {
	...
	if delayAct.action == v1alpha1.RestartTaskAction {
		...
		action.Target = state.Target{TaskName: delayAct.taskName, PodName: delayAct.podName, PartitionGroupName: delayAct.partitionGroup, Type: state.TargetTypePartition}
	}
...
func GetActionType(action v1alpha1.Action) ActionType {
	...
	case v1alpha1.RestartPartitionAction:
		return PartitionGroupAction
	}
	return JobAction
}

func getPodPartitionGroup(pod *v1.Pod) string {
