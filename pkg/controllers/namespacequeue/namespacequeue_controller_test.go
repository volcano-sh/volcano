...
func TestAddNamespaceQueueRequeuesParentTree(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	parent := newTestNamespaceQueue("team-a", "department", "cluster/research")
	queue := newTestNamespaceQueue("team-a", "training", parent.Name)
	sibling := newTestNamespaceQueue("team-a", "inference", parent.Name)
	for _, item := range []*schedulingv1beta1.NamespaceQueue{parent, queue, sibling} {
		if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(item); err != nil {
			t.Fatalf("failed to add NamespaceQueue %s/%s: %v", item.Namespace, item.Name, err)
		}
	}

	controller.addNamespaceQueue(queue)
...
func TestUpdateNamespaceQueueRequeuesDeletingRuntimeChange(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	oldQueue := newTestNamespaceQueue("team-a", "training", "cluster/research")
	oldQueue.ResourceVersion = "1"
	now := metav1.Now()
	oldQueue.DeletionTimestamp = &now
	oldQueue.Status.State = schedulingv1beta1.QueueStateClosing
	newQueue := oldQueue.DeepCopy()
	newQueue.ResourceVersion = "2"
	newQueue.Status.Allocated = v1.ResourceList{
...
func TestRecordConditionEvents(t *testing.T) {
	...
	if !conditionEventChanged(&newQueue.Status.Conditions[0], nil) {
		t.Fatal("condition removal was not detected")
	}
	controller.recordConditionEvents(newQueue, oldQueue)
	select {
	case event := <-recorder.Events:
		t.Fatalf("condition removal emitted event: %s", event)
	default:
	}
}
...
func TestDeleteNamespaceQueueRequeuesParentTree(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	parent := newTestNamespaceQueue("team-a", "department", "cluster/research")
	sibling := newTestNamespaceQueue("team-a", "inference", parent.Name)
	for _, item := range []*schedulingv1beta1.NamespaceQueue{parent, sibling} {
		if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(item); err != nil {
			t.Fatalf("failed to add NamespaceQueue %s/%s: %v", item.Namespace, item.Name, err)
		}
	}
	deleted := newTestNamespaceQueue("team-a", "training", parent.Name)

	controller.deleteNamespaceQueue(deleted)
	expectWorkQueueKeys(t, controller, []string{
		"team-a/department",
		"team-a/inference",
		"team-a/training",
...
func TestDeleteNamespaceQueueRequeuesDescendantsWhenParentReferenceIsInvalid(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	child := newTestNamespaceQueue("team-a", "child", "parent")
	...
	deleted := newTestNamespaceQueue("team-a", "parent", "")
	controller.deleteNamespaceQueue(deleted)
	expectWorkQueueKeys(t, controller, []string{
		"team-a/child",
		"team-a/parent",
...
func TestAddNamespaceQueueRequeuesDescendantsWhenParentReferenceIsInvalid(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	child := newTestNamespaceQueue("team-a", "child", "parent")
	if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(child); err != nil {
		t.Fatalf("failed to add child NamespaceQueue: %v", err)
	...
	added := newTestNamespaceQueue("team-a", "parent", "")
	controller.addNamespaceQueue(added)
	expectWorkQueueKeys(t, controller, []string{
		"team-a/child",
		"team-a/parent",
	})
...
func TestAddQueueRequeuesOnlyDirectChildrenOfQueueParent(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	for _, queue := range []*schedulingv1beta1.NamespaceQueue{
		newTestNamespaceQueue("team-a", "direct", "cluster/default"),
		newTestNamespaceQueue("team-a", "grandchild", "direct"),
	} {
		if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(queue); err != nil {
			t.Fatalf("failed to add NamespaceQueue %s/%s: %v", queue.Namespace, queue.Name, err)
		}
	...
	controller.addQueue(&schedulingv1beta1.Queue{
		ObjectMeta: metav1.ObjectMeta{Name: "research"},
		Spec:       schedulingv1beta1.QueueSpec{Parent: "default"},
	})

	expectWorkQueueKey(t, controller, "team-a/direct")
	if controller.workQueue.Len() != 0 {
		t.Fatalf("workQueue contains descendants beyond the direct parent child")
	}
}
...
func TestEnqueueDescendantNamespaceQueuesIncludesClusterQueueDescendants(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	clusterQueues := []*schedulingv1beta1.Queue{
		{
			ObjectMeta: metav1.ObjectMeta{Name: "research", ResourceVersion: "1"},
			Spec:       schedulingv1beta1.QueueSpec{Parent: "root"},
		},
		{
			ObjectMeta: metav1.ObjectMeta{Name: "research-gpu", ResourceVersion: "1"},
			Spec:       schedulingv1beta1.QueueSpec{Parent: "research"},
		},
		{
	...
	for _, queue := range clusterQueues {
		if err := controller.queueInformer.Informer().GetIndexer().Add(queue); err != nil {
			t.Fatalf("failed to add Cluster Queue %s: %v", queue.Name, err)
		}
	}

	namespaceQueues := []*schedulingv1beta1.NamespaceQueue{
		newTestNamespaceQueue("team-a", "training", "cluster/research-gpu"),
		newTestNamespaceQueue("team-a", "task", "training"),
		newTestNamespaceQueue("team-b", "inference", "cluster/research"),
		newTestNamespaceQueue("team-c", "other", "cluster/other"),
	}
	for _, queue := range namespaceQueues {
		if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(queue); err != nil {
			t.Fatalf("failed to add NamespaceQueue %s/%s: %v", queue.Namespace, queue.Name, err)
		}
	}
...
func TestUpdateQueueRequeuesOldAndNewClusterParentTrees(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	for _, queue := range []*schedulingv1beta1.NamespaceQueue{
		newTestNamespaceQueue("team-a", "old-child", "cluster/old-parent"),
		newTestNamespaceQueue("team-b", "new-child", "cluster/new-parent"),
	} {
		if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(queue); err != nil {
			t.Fatalf("failed to add NamespaceQueue %s/%s: %v", queue.Namespace, queue.Name, err)
		}
	...
	oldQueue := &schedulingv1beta1.Queue{
		ObjectMeta: metav1.ObjectMeta{Name: "moving", ResourceVersion: "1"},
		Spec:       schedulingv1beta1.QueueSpec{Parent: "old-parent"},
	}
	newQueue := oldQueue.DeepCopy()
	newQueue.ResourceVersion = "2"
	newQueue.Spec.Parent = "new-parent"
...
func TestIsNamespaceQueueReadyRequiresCurrentGeneration(t *testing.T) {
	...
	if controllerutil.IsNamespaceQueueSchedulable(queue.Generation, string(queue.Status.State), queue.Status.Conditions) {
		t.Fatal("isNamespaceQueueReady() = true for a stale Ready condition")
	}

	for index := range queue.Status.Conditions {
		queue.Status.Conditions[index].ObservedGeneration = queue.Generation
	}
	if !controllerutil.IsNamespaceQueueSchedulable(queue.Generation, string(queue.Status.State), queue.Status.Conditions) {
...
func TestUpdateNamespaceQueueStatusPreservesSchedulerOwnedFields(t *testing.T) {
	...
	if _, err := controller.updateNamespaceQueueStatus(nq); err != nil {
...
func TestUpdateNamespaceQueueStatusUsesLatestRuntimeState(t *testing.T) {
	...
	if _, err := controller.updateNamespaceQueueStatus(stale); err != nil {
		t.Fatalf("updateNamespaceQueueStatus() error = %v", err)
	}
...
func TestNamespaceQueueParentRecoversAfterChildResourceUpdate(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	clusterParent := &schedulingv1beta1.Queue{
		ObjectMeta: metav1.ObjectMeta{Name: "research"},
		Spec: schedulingv1beta1.QueueSpec{
			AllowedNamespaces: []string{"team-a"},
			Guarantee: schedulingv1beta1.Guarantee{
				Resource: v1.ResourceList{v1.ResourceCPU: resource.MustParse("2")},
			},
			Deserved: v1.ResourceList{v1.ResourceCPU: resource.MustParse("2")},
		},
	}
	parent := newTestNamespaceQueue("team-a", "department", "cluster/research")
	parent.Spec.Guarantee.Resource = v1.ResourceList{v1.ResourceCPU: resource.MustParse("1")}
	parent.Spec.Deserved = v1.ResourceList{v1.ResourceCPU: resource.MustParse("1")}
	child := newTestNamespaceQueue("team-a", "training", parent.Name)
	child.ResourceVersion = "1"
	child.Spec.Guarantee.Resource = v1.ResourceList{v1.ResourceCPU: resource.MustParse("2")}
	child.Spec.Deserved = v1.ResourceList{v1.ResourceCPU: resource.MustParse("2")}

	if err := controller.queueInformer.Informer().GetIndexer().Add(clusterParent); err != nil {
		t.Fatalf("failed to add cluster Queue: %v", err)
	}
	for _, queue := range []*schedulingv1beta1.NamespaceQueue{parent, child} {
		if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(queue); err != nil {
			t.Fatalf("failed to add NamespaceQueue %s/%s: %v", queue.Namespace, queue.Name, err)
		}
	}
	...
	if err := controller.syncNamespaceQueue("team-a/department"); err != nil {
		t.Fatalf("failed to reconcile overcommitted parent NamespaceQueue: %v", err)
	}
	assertReadyReason(t, controller, "team-a", "department", "ParentConstraintViolation")

	updatedChild := child.DeepCopy()
	updatedChild.ResourceVersion = "2"
	updatedChild.Spec.Guarantee.Resource[v1.ResourceCPU] = resource.MustParse("500m")
	updatedChild.Spec.Deserved[v1.ResourceCPU] = resource.MustParse("500m")
	if err := controller.namespaceQueueInformer.Informer().GetIndexer().Update(updatedChild); err != nil {
		t.Fatalf("failed to update child NamespaceQueue: %v", err)
	}
	if _, err := controller.vcClient.SchedulingV1beta1().NamespaceQueues(updatedChild.Namespace).
		Update(context.Background(), updatedChild.DeepCopy(), metav1.UpdateOptions{}); err != nil {
		t.Fatalf("failed to update child NamespaceQueue in fake client: %v", err)
	}
	controller.updateNamespaceQueue(child, updatedChild)
	for controller.workQueue.Len() > 0 {
		if !controller.processNextWorkItem() {
			t.Fatal("work queue shut down while reconciling child resource update")
		}
	}

	updatedParent, err := controller.vcClient.SchedulingV1beta1().NamespaceQueues(parent.Namespace).
		Get(context.Background(), parent.Name, metav1.GetOptions{})
	if err != nil {
		t.Fatalf("failed to get parent NamespaceQueue: %v", err)
	}
	ready := apiMeta.FindStatusCondition(updatedParent.Status.Conditions, controllerutil.NamespaceQueueReadyCondition)
	if ready == nil || ready.Status != metav1.ConditionTrue {
		t.Fatalf("Ready condition = %#v, want True after child resources are reduced", ready)
	}
}

func TestMultipleNamespaceQueuesMayAttachToOneClusterQueue(t *testing.T) {
	controller := newFakeNamespaceQueueController(t)
	parent := &schedulingv1beta1.Queue{
		ObjectMeta: metav1.ObjectMeta{Name: "research"},
	...
	queue := newNamespaceQueue("team-a", "cluster/research")
	queue.Name = "training"
	sibling := newNamespaceQueue("team-a", "cluster/research")
	sibling.Name = "inference"
	addNamespaceQueueTestObjects(t, controller, queue, parent)
	if err := controller.namespaceQueueInformer.Informer().GetIndexer().Add(sibling); err != nil {
		t.Fatalf("failed to add sibling NamespaceQueue: %v", err)
	}
	...
	ready := apiMeta.FindStatusCondition(updated.Status.Conditions, controllerutil.NamespaceQueueReadyCondition)
	if ready == nil || ready.Status != metav1.ConditionTrue {
