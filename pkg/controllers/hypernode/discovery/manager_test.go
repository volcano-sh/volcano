...
func TestManager_StartMultipleDiscoverers(t *testing.T) {
	...
	constructorA := api.DiscovererConstructor(func(cfg api.DiscoveryConfig) api.Discoverer {
		return fakedisc.NewFakeDiscoverer(hyperNodesA, cfg)
	})
	constructorB := api.DiscovererConstructor(func(cfg api.DiscoveryConfig) api.Discoverer {
	...
	mockLoader := config.NewFakeLoader(discoveryConfig)

	// Create manager
	queue := workqueue.NewTypedRateLimitingQueue(workqueue.DefaultTypedControllerRateLimiter[string]())
	m := NewManager(mockLoader, queue)
	err := m.Start()
...
func TestManager_syncHandler(t *testing.T) {
	...
	constructor := api.DiscovererConstructor(func(cfg api.DiscoveryConfig) api.Discoverer {
