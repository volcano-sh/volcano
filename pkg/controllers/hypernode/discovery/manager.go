...
func NewManager(configLoader config.Loader, queue workqueue.TypedRateLimitingInterface[string]) Manager {
	return &manager{
...
// startAllDiscoverers initializes and starts all enabled network discoverers
func (m *manager) startAllDiscoverers() error {
	m.mutex.Lock()
	defer m.mutex.Unlock()

	sources := m.config.GetEnabledDiscoverySources()
	if len(sources) == 0 {
		klog.InfoS("No enabled network topology discovery source found in configuration")
		return nil
	}

	for _, source := range sources {
		if err := m.startSingleDiscoverer(source); err != nil {
			klog.ErrorS(err, "Failed to start discoverer", "source", source)
			continue
		}
	}

	klog.InfoS("Successfully started all network topology discoverers")
	return nil
}
...
func (m *manager) startSingleDiscoverer(source string) error {
	...
	discoverer, err := api.NewDiscoverer(*discoveryCfg)
	if err != nil {
...
func (m *manager) stopSingleDiscoverer(source string) error {
	...
	if !exists {
		klog.InfoS("Failed to stop discoverer", "source", source, "reason", "discoverer not found")
		return nil
	}
