...
type CNIPluginsConfHandler interface {
	...
	AddOrUpdateCniPluginToConfList(configPath, name string, plugin map[string]any) (err error)
...
func (p *ConfHandler) AddOrUpdateCniPluginToConfList(configPath, name string, plugin map[string]any) (err error) {
	...
	plugins, ok := list["plugins"].([]any)
	if !ok {
		return fmt.Errorf("failed to get plugins from file")
	}

	var index int
	var existedPlugin map[string]any
	for index = 0; index < len(plugins); index++ {
		existedPlugin, ok = plugins[index].(map[string]any)
		if !ok {
...
func (p *ConfHandler) DeleteCniPluginFromConfList(configPath, name string) (err error) {
	...
	var index int
	for index = 0; index < len(plugins); index++ {
		pl, ok := plugins[index].(map[string]any)
		if !ok {
			return fmt.Errorf("failed to get cni from cni conflist")
		}
