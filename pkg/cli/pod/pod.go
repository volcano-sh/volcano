...
func ListPods(ctx context.Context) error {
	...
	if listPodFlags.JobName != "" {
		...
		if listPodFlags.QueueName != "" {
			...
			if !checkIsMatchLabel(listVcjobPodsRes, v1alpha1.QueueNameKey, listPodFlags.QueueName) {
		...
		listVcJobPodsRes := filterByLabelFunc(listAllPodsRes, v1alpha1.QueueNameKey, listPodFlags.QueueName)

		// then, filter all other workload pods belong to the queue
		listNormalPodsRes := filterByAnnotationFunc(listAllPodsRes, schedulingv1beta1.QueueNameAnnotationKey, listPodFlags.QueueName)
...
// checkIsMatchLabel check if the pods match the labelKey and labelValue
func checkIsMatchLabel(pods *corev1.PodList, labelKey, labelValue string) bool {
	for _, pod := range pods.Items {
...
// filterByLabelFunc filter pods based on labelKey and labelValue
func filterByLabelFunc(pods *corev1.PodList, labelKey, labelValue string) *corev1.PodList {
	filteredPods := &corev1.PodList{}
	for _, pod := range pods.Items {
		if value, exist := pod.Labels[labelKey]; exist {
			if value == labelValue {
