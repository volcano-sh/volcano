...
func TestValidateNamespaceQueueParentChange(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "legacy empty parent can be repaired",
		...
		{
			name:      "new empty parent is invalid",
			oldQueue:  &schedulingv1beta1.NamespaceQueue{ObjectMeta: metav1.ObjectMeta{Namespace: "team-a"}, Spec: schedulingv1beta1.NamespaceQueueSpec{Parent: "cluster/research"}},
			newParent: "",
			wantErr:   true,
		},
		{
...
func TestIsNamespaceQueueSchedulable(t *testing.T) {
	conditions := func(authorized, ready metav1.ConditionStatus, generation int64) []metav1.Condition {
		return []metav1.Condition{
			{Type: NamespaceQueueAuthorizedCondition, Status: authorized, ObservedGeneration: generation},
			{Type: NamespaceQueueReadyCondition, Status: ready, ObservedGeneration: generation},
		}
	}
	...
	tests := []struct {
		...
		{
			name:       "current authorized and ready conditions",
			observed:   "Open",
			generation: 1,
		...
		},
		{
			...
			observed:   "Open",
			generation: 1,
			conditions: conditions(metav1.ConditionUnknown, metav1.ConditionTrue, 1),
		},
		{
			name:       "ready is required",
			observed:   "Open",
			generation: 1,
			conditions: conditions(metav1.ConditionTrue, metav1.ConditionFalse, 1),
		},
		{
			name:       "stale conditions are rejected",
			observed:   "Open",
			generation: 2,
			conditions: conditions(metav1.ConditionTrue, metav1.ConditionTrue, 1),
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsNamespaceQueueSchedulable(tt.generation, tt.observed, tt.conditions); got != tt.want {
				t.Fatalf("IsNamespaceQueueSchedulable() = %t, want %t", got, tt.want)
			}
		})
	}
}

func TestIsNamespaceAllowedByQueue(t *testing.T) {
	tests := []struct {
		name        string
		...
		{
			name:        "wildcard allows every namespace",
			queue:       &schedulingv1beta1.Queue{Spec: schedulingv1beta1.QueueSpec{AllowedNamespaces: []string{"*"}}},
			namespace:   "team-a",
			wantAllowed: true,
		},
		{
			name:        "explicit namespace is allowed",
			queue:       &schedulingv1beta1.Queue{Spec: schedulingv1beta1.QueueSpec{AllowedNamespaces: []string{"team-a"}}},
			namespace:   "team-a",
			wantAllowed: true,
		},
		{
			name:        "other namespace is rejected",
			queue:       &schedulingv1beta1.Queue{Spec: schedulingv1beta1.QueueSpec{AllowedNamespaces: []string{"team-a"}}},
			namespace:   "team-b",
			wantAllowed: false,
		},
		{
			name:        "nil queue is rejected",
			namespace:   "team-a",
			wantAllowed: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsNamespaceAllowedByQueue(tt.queue, tt.namespace); got != tt.wantAllowed {
				t.Fatalf("IsNamespaceAllowedByQueue() = %t, want %t", got, tt.wantAllowed)
			}
		})
	}
}
