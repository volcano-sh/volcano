	...
	err := retryOnConnectionRefused(func() error {
		return forMetricMap(func(m map[string]float64) {
			...
			assert.Contains(t, m, "volcano_queue_inqueue_scalar_resources{queue_name=\"queue4\",resource=\"nvidia.com/gpu\"}",
				"volcano_queue_inqueue_scalar_resources for queue4 and resource nvidia.com/gpu should be present")
			assert.Equal(t, 3., m["volcano_queue_inqueue_scalar_resources{queue_name=\"queue4\",resource=\"nvidia.com/gpu\"}"])
		})
	})
	if err != nil {
	...
	err = retryOnConnectionRefused(func() error {
		return forMetricMap(func(m map[string]float64) {
			...
			assert.NotContains(t, m, "volcano_queue_inqueue_memory_bytes{queue_name=\"queue4\"}",
				"volcano_queue_inqueue_memory_bytes for queue4 should be removed")
			assert.NotContains(t, m, "volcano_queue_inqueue_scalar_resources{queue_name=\"queue4\",resource=\"nvidia.com/gpu\"}",
				"volcano_queue_inqueue_scalar_resources for queue4 should be removed")
