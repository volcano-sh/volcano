...
func (cp *capacityPlugin) buildHierarchicalQueueAttrs(ssn *framework.Session) bool {
	...
	cp.checkHierarchicalQueue(rootQueueAttr, ssn)
...
// recordQueueEvent sends a K8s event to the queue if it exists in the session
func (cp *capacityPlugin) recordQueueEvent(attr *queueAttr, ssn *framework.Session, eventType, reason, message string) {
	if queueInfo, found := ssn.Queues[attr.queueID]; found {
		ssn.RecordQueueEvent(queueInfo, eventType, reason, message)
	}
}

func (cp *capacityPlugin) checkHierarchicalQueue(attr *queueAttr, ssn *framework.Session) {
	totalGuarantee := api.EmptyResource()
	...
	for _, childAttr := range attr.children {
		...
		if attr.capability.LessPartly(childAttr.capability, api.Zero) {
			msg := fmt.Sprintf("Child queue %s capability (%s) exceeds parent capability (%s). "+
				"Child's effective capability will be limited by parent.",
				childAttr.name, childAttr.capability, attr.capability)
			cp.recordQueueEvent(attr, ssn, v1.EventTypeWarning, "CapabilityExceeded", msg)
		}
	}
	...
	if attr.deserved.LessPartly(totalDeserved, api.Zero) {
		msg := fmt.Sprintf("Sum of child queue deserved (%s) exceeds parent deserved (%s). "+
			"This may affect resource distribution during scheduling.",
			totalDeserved, attr.deserved)

		cp.recordQueueEvent(attr, ssn, v1.EventTypeWarning, "DeservedExceeded", msg)
	}
	...
	if attr.guarantee.LessPartly(totalGuarantee, api.Zero) {
		msg := fmt.Sprintf("Sum of child queue guarantees (%s) exceeds parent guarantee (%s). "+
			"Not all child guarantees can be satisfied simultaneously.",
			totalGuarantee, attr.guarantee)

		cp.recordQueueEvent(attr, ssn, v1.EventTypeWarning, "GuaranteeExceeded", msg)
	}

	// Recursively check child queues
	for _, childAttr := range attr.children {
		cp.checkHierarchicalQueue(childAttr, ssn)
	}
}
