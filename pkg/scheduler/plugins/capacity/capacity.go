...
func (cp *capacityPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	ssn.AddReclaimableFn(cp.Name(), func(reclaimer *api.TaskInfo, reclaimees []*api.TaskInfo) ([]*api.TaskInfo, int) {
		...
		for _, reclaimee := range reclaimees {
			...
			if job == nil {
				klog.V(4).Infof("[capacity] Skip reclaimee <%s/%s>: job <%s> not found in session (orphaned task from deleted PodGroup)",
			...
			if attr == nil {
				klog.V(4).Infof("[capacity] Skip reclaimee <%s/%s>: queue <%s> not found in queueOpts",
	...
	ssn.AddEventHandler(&framework.EventHandler{
		AllocateFunc: func(event *framework.Event) {
			...
			if job == nil {
				klog.V(4).Infof("[capacity] Skip allocate event for task <%s/%s>: job <%s> not found in session (orphaned task from deleted PodGroup)",
					event.Task.Namespace, event.Task.Name, event.Task.Job)
				return
			}
			attr := cp.queueOpts[job.Queue]
			if attr == nil {
				klog.V(4).Infof("[capacity] Skip allocate event for task <%s/%s>: queue <%s> not found in queueOpts",
		...
		DeallocateFunc: func(event *framework.Event) {
			job := ssn.Jobs[event.Task.Job]
			if job == nil {
				klog.V(4).Infof("[capacity] Skip deallocate event for task <%s/%s>: job <%s> not found in session (orphaned task from deleted PodGroup)",
					event.Task.Namespace, event.Task.Name, event.Task.Job)
				return
			}
			attr := cp.queueOpts[job.Queue]
			if attr == nil {
				klog.V(4).Infof("[capacity] Skip deallocate event for task <%s/%s>: queue <%s> not found in queueOpts",
					event.Task.Namespace, event.Task.Name, job.Queue)
				return
			}
