...
func (cp *capacityPlugin) buildQueueAttrs(ssn *framework.Session) {
	cp.addClusterQueueGuarantees(ssn.Queues)
...
func (cp *capacityPlugin) addClusterQueueGuarantees(queues map[api.QueueID]*api.QueueInfo) {
	for _, queue := range queues {
		if queue == nil || queue.Scope != api.ClusterQueueScope || len(queue.Guarantee.Resource) == 0 {
			continue
		}
		cp.totalGuarantee.Add(api.NewResource(queue.Guarantee.Resource))
	}
}
