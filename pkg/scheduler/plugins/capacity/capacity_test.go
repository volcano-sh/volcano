...
func Test_capacityPlugin_OnSessionOpenWithHierarchy(t *testing.T) {
	...
	// queue
	queue8 := buildQueueWithParents("q8", "root", nil, api.BuildResourceList("4", "4Gi"))
	queue81 := buildQueueWithParents("q81", "q8", api.BuildResourceList("4", "3Gi"), api.BuildResourceList("3", "3Gi"))

	// resources for test case 9
	// queue
	queue9 := buildQueueWithParents("q9", "root", api.BuildResourceList("3", "5Gi"), api.BuildResourceList("4", "4Gi"))
	queue91 := buildQueueWithParents("q91", "q9", api.BuildResourceList("2", "2Gi"), api.BuildResourceList("3", "3Gi"))
	...
	tests := []uthelper.TestCommonStruct{
		...
		{
			Name:    "case8: queue is not valid when capability less than deserved ",
			Plugins: plugins,
			Nodes:   []*corev1.Node{n1},
			Queues:  []*schedulingv1beta1.Queue{root, queue8, queue81},
		},
		{
			Name:    "case9: queue is not valid when parent queue is not valid",
			Plugins: plugins,
			Nodes:   []*corev1.Node{n1},
			Queues:  []*schedulingv1beta1.Queue{root, queue91, queue9},
		},
	}
