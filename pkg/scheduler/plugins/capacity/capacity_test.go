...
func TestBuildQueueAttrsExcludesNamespaceQueueGuarantee(t *testing.T) {
	clusterQueue := &api.QueueInfo{
		UID:   api.QueueID("cluster"),
	...
	namespaceQueue := &api.QueueInfo{
		...
		Guarantee: scheduling.Guarantee{
			Resource: corev1.ResourceList{corev1.ResourceCPU: resource.MustParse("8")},
		},
	}

	cp := &capacityPlugin{
		totalResource:  api.NewResource(api.BuildResourceList("16", "16Gi")),
		totalGuarantee: api.EmptyResource(),
		queueOpts:      map[api.QueueID]*queueAttr{},
	}
	cp.addClusterQueueGuarantees(map[api.QueueID]*api.QueueInfo{
		clusterQueue.UID:   clusterQueue,
		namespaceQueue.UID: namespaceQueue,
	})

	want := api.NewResource(clusterQueue.Guarantee.Resource)
	if cp.totalGuarantee.MilliCPU != want.MilliCPU {
		t.Fatalf("total guarantee = %v, want %v", cp.totalGuarantee, want)
	}
}
