...
func TestProportionHints(t *testing.T) {
	...
	queueWithGuarantee := func(guarantee string) *scheduling.Queue {
		q := queue(scheduling.QueueStateOpen, 1, "")
		if guarantee != "" {
	...
	podGroup := func(queue string, minResources v1.ResourceList, phase scheduling.PodGroupPhase) *api.PodGroup {
	...
	tests := []struct {
		...
		{name: "queue state change wakes any rejected Job", hintFn: queueHint, oldObj: queue(scheduling.QueueStateClosed, 1, ""), newObj: queue(scheduling.QueueStateOpen, 1, ""), want: unschedulable.HintWakeup},
		...
		{name: "queue guarantee change wakes any rejected Job", hintFn: queueHint, oldObj: queueWithGuarantee("1"), newObj: queueWithGuarantee("2"), want: unschedulable.HintWakeup},
		...
		{name: "consuming PodGroup releasing CPU wakes Job", hintFn: podGroupHint, oldObj: podGroup("queue-a", cpu, scheduling.PodGroupRunning), newObj: podGroup("queue-a", cpu, scheduling.PodGroupCompleted), want: unschedulable.HintWakeup},
		{name: "consuming PodGroup moving Queue wakes Job", hintFn: podGroupHint, oldObj: podGroup("queue-a", cpu, scheduling.PodGroupRunning), newObj: podGroup("queue-b", cpu, scheduling.PodGroupRunning), want: unschedulable.HintWakeup},
		{name: "PodGroup releasing only memory is skipped", hintFn: podGroupHint, oldObj: podGroup("queue-a", memory, scheduling.PodGroupRunning), newObj: podGroup("queue-a", memory, scheduling.PodGroupCompleted), want: unschedulable.HintSkip},
		{name: "non-consuming PodGroup deletion is skipped", hintFn: podGroupHint, oldObj: podGroup("queue-a", cpu, scheduling.PodGroupPending), want: unschedulable.HintSkip},
	}
