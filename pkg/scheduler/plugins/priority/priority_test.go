...
var (
	...
	falseValue       = false
	priority         = int32(1000)
	prioPluginEnable = conf.PluginOption{
		Name:               PluginName,
	...
	prioPluginDisable = conf.PluginOption{
		Name:               PluginName,
		EnabledTaskOrder:   &trueValue,
		EnabledJobOrder:    &trueValue,
...
func TestReclaim(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			tiers: []conf.Tier{
				{
					Plugins: []conf.PluginOption{prioPluginEnable},
				},
			},
			TestCommonStruct: uthelper.TestCommonStruct{
				...
				Evicted:   []string{"ns1/reclaimee1"},
				EvictNum:  1,
				PipeLined: map[string][]string{"ns2/pg2": {"node1"}},
			},
		},
		{
			actions: actions,
			tiers: []conf.Tier{
				{
					Plugins: []conf.PluginOption{prioPluginDisable},
				},
			},
			TestCommonStruct: uthelper.TestCommonStruct{
		...
		{ // test allocate task order and job order
			...
			TestCommonStruct: uthelper.TestCommonStruct{
				...
				BindsNum: 1, EvictNum: 0,
				Bind:    map[string]string{"ns1/pod2": "node1"},
				Evicted: []string{}, PipeLined: map[string][]string{},
			},
		},
	}
	for i, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			test.RegistSession(test.tiers, nil)
			defer test.Close()
