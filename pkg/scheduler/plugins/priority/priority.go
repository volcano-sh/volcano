...
func (pp *priorityPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	preemptableFn := func(preemptor *api.TaskInfo, preemptees []*api.TaskInfo) ([]*api.TaskInfo, int) {
		...
		if preemptorJob == nil {
			klog.V(4).Infof("[priority] Skip preemption: preemptor job <%s> not found in session (orphaned task from deleted PodGroup)",
				preemptor.Job)
		...
		for _, preemptee := range preemptees {
			...
			if preempteeJob == nil {
				klog.V(4).Infof("[priority] Skip preemptee <%s/%s>: job <%s> not found in session (orphaned task from deleted PodGroup)",
