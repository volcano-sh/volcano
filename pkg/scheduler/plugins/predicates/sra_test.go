...
func TestArguments(t *testing.T) {
	...
	arguments := framework.Arguments{
		...
		"predicate.resources.nvidia.com/t4.cpu":     3,
		"predicate.resources.nvidia.com/t4.memory":  6,
		"predicate.resources.nvidia.com/a10.cpu":    4,
		"predicate.resources.nvidia.com/a10.memory": 8,
		"predicate.sraEnable":                       true,
		"predicate.sra.weight":                      3,
		"predicate.sra.resources.nvidia.com/t4":     1,
		"predicate.sra.resources.nvidia.com/a10":    2,
	}
	...
	pp, ok := plugin.(*predicatesPlugin)
	if !ok {
		t.Fatalf("plugin should be %T, but not %T", pp, plugin)
	}

	predicate := enablePredicate(pp.pluginArguments)
	sra := predicate.sra
	if sra.Weight != 3 {
		t.Errorf("sra weight should be 3, but not %v", sra.Weight)
	}
	if sra.ResourcesWeightSum != 3 {
		t.Errorf("the sum of resource weights of sra should be 3, but not %v", sra.ResourcesWeightSum)
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			arguments: framework.Arguments{
				"predicate.resources":                    "nvidia.com/t4, nvidia.com/a10",
				"predicate.sraEnable":                    true,
				"predicate.sra.weight":                   5,
				"predicate.sra.resources.nvidia.com/t4":  2,
				"predicate.sra.resources.nvidia.com/a10": 3,
			},
			expected: map[string]map[string]float64{
				"c1/p1": {
					"n1": 500,
		...
		},
		{
			...
			arguments: framework.Arguments{
				"predicate.resources":                   "nvidia.com/t4",
				"predicate.sraEnable":                   true,
				"predicate.sra.weight":                  5,
				"predicate.sra.resources.nvidia.com/t4": 2,
			},
