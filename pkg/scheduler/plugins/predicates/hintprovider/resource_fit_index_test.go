...
func TestResourceFitNodeAddJobKeysRequiresCompleteTaskData(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "rejects a task without InitResreq",
			tasks: []api.TaskID{completeTask.UID, incompleteTask.UID},
		},
		{
			name:  "rejects a task missing from the Job",
			tasks: []api.TaskID{completeTask.UID, "missing"},
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := entry.JobKeysFn(job, unschedulable.Rejection{Tasks: test.tasks})
			assert.Error(t, err)
		})
	}
}
