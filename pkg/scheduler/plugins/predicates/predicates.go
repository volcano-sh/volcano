...
func (pp *predicatesPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	ssn.AddPredicateFn(pp.Name(), func(task *api.TaskInfo, node *api.NodeInfo) ([]*api.Status, error) {
		...
		if predicate.volumeBindingEnable {
			...
			volumeBindingStatus, err := framework.ConvertPredicateStatus(status)
			predicateStatus = append(predicateStatus, volumeBindingStatus)
			if err != nil {
				return predicateStatus, fmt.Errorf("plugin %s pre-predicates failed %s", volumeBindingFilter.Name(), status.Message())
			}
			status = volumeBindingFilter.Filter(context.TODO(), state, task.Pod, nodeInfo)
			volumeBindingStatus, err = framework.ConvertPredicateStatus(status)
			predicateStatus = append(predicateStatus, volumeBindingStatus)
			if err != nil {
				return predicateStatus, fmt.Errorf("plugin %s predicates failed %s", volumeBindingFilter.Name(), status.Message())
			}
		}
