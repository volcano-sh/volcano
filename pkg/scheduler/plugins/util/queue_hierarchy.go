...
// BuildEffectiveQueueHierarchy fills session-local hierarchy fields used by
// hierarchical plugins. Existing cluster Queue annotations are preserved;
// NamespaceQueues are derived from canonical parent IDs. The function mutates
// only the supplied session-local QueueInfo values and returns an error for a
// missing parent or cycle instead of producing a partial hierarchy.
