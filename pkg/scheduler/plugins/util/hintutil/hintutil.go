...
func ResourceLimitRelaxed(oldLimit, newLimit v1.ResourceList) bool {
	...
	// Limits that remain are looser only when their quantity increases. A newly
	// added dimension introduces a limit and therefore cannot relax the old one.
	for name, oldQuantity := range oldLimit {
		if newQuantity, limited := newLimit[name]; limited && newQuantity.Cmp(oldQuantity) > 0 {
			return true
		}
	}
	return false
