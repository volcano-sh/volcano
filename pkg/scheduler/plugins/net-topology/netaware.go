...
const (
	PluginName            = "net-topology"
	networkTopologyWeight = "net-topology.weight"
	networkTopologyType   = "net-topology.type" // strategy type for network topology
	// strategy value for net-topology.type
	staticAware  = "static"
...
type netTopPlugin struct {
	...
	weight          int            // net-topology plugin score weight
	topolgyType     string         // supported topology type: static, dynamic
...
func (np *netTopPlugin) parseArguments() {
	...
	np.topolgyType = strings.TrimSpace(v)
}
...
func (np *netTopPlugin) parseStaticAwareArguments() bool {
	...
	if !exist {
		klog.Warningf("plugin %s (with type %s) arguments does not configure %s, skip", PluginName, np.topolgyType, networkTopologyKeys)
		return false
	}
...
func (np *netTopPlugin) OnSessionOpen(ssn *framework.Session) {
	np.parseArguments()
	if np.topolgyType == staticAware {
		valid := np.parseStaticAwareArguments()
		if valid {
			np.staticTopAware.OnSessionOpen(ssn)
		}
	} else {
		klog.Warningf("strategy %s is not supported in plugin %s", np.topolgyType, PluginName)
	}
}
