...
func (np *nodeGroupPlugin) OnSessionOpen(ssn *framework.Session) {
	...
	nodeOrderFn := func(task *api.TaskInfo, node *api.NodeInfo) (float64, error) {
		...
		if job == nil {
			klog.V(4).Infof("[nodegroup] Skip node scoring for task <%s/%s>: job <%s> not found in session (orphaned task from deleted PodGroup)",
	...
	predicateFn := func(task *api.TaskInfo, node *api.NodeInfo) error {
		job := ssn.Jobs[task.Job]
		if job == nil {
			klog.V(4).Infof("[nodegroup] Skip predicate for task <%s/%s>: job <%s> not found in session (orphaned task from deleted PodGroup)",
				task.Namespace, task.Name, task.Job)
