...
func TestJobCacheRecordAndGet(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			registered:     []string{"plugin"},
			rejections:     []Rejection{{Plugin: "plugin", Source: RejectionPredicate, Tasks: []api.TaskID{"task"}}},
			wantRejections: []Rejection{{Plugin: "plugin", Source: RejectionPredicate, Tasks: []api.TaskID{"task"}}},
		},
		{
		...
		{
			name:       "does not cache when any rejecting plugin has no hints",
			registered: []string{"registered"},
			rejections: []Rejection{
				{Plugin: "registered", Source: RejectionPredicate, Tasks: []api.TaskID{"task-a"}},
				{Plugin: "unregistered", Source: RejectionPredicate, Tasks: []api.TaskID{"task-b"}},
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, pluginName := range test.registered {
				registerTestHint(registry, pluginName, fwk.ClusterEvent{Resource: fwk.Node, ActionType: fwk.Add}, nil)
			}

			cache.Record(job, test.rejections)
...
func TestJobCacheRecordsAcrossSessionEvents(t *testing.T) {
	registeredEvent := fwk.ClusterEvent{Resource: fwk.Pod, ActionType: fwk.Delete}
	tests := []struct {
		name       string
		event      fwk.ClusterEvent
		wantCached bool
		...
		{
			name:  "matching event prevents recording",
			event: registeredEvent,
		},
		{
			name:       "unrelated event allows recording",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			job := api.NewJobInfo("job")
			cache, registry := newTestUnschedulableCache()
			registerTestHint(registry, "plugin", registeredEvent, func(*api.JobInfo, Rejection, any, any) (HintResult, error) {
				return HintSkip, nil
			})
...
func TestJobCacheOnEventCombinesMatchingHints(t *testing.T) {
	tests := []struct {
		name       string
		results    []HintResult
		wantCached bool
		...
		{
			name:       "all skip results keep the record",
			results:    []HintResult{HintSkip, HintSkip},
			wantCached: true,
		},
		{
			name:    "a wakeup result removes the record",
			results: []HintResult{HintSkip, HintWakeup},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			cache, registry := newTestUnschedulableCache()
			event := fwk.ClusterEvent{Resource: fwk.Node, ActionType: fwk.Add}
			rejections := make([]Rejection, 0, len(test.results))
			for i, result := range test.results {
				pluginName := fmt.Sprintf("plugin-%d", i)
				hintResult := result
				registerTestHint(registry, pluginName, event, func(*api.JobInfo, Rejection, any, any) (HintResult, error) {
					return hintResult, nil
				})
				rejections = append(rejections, Rejection{Plugin: pluginName, Source: RejectionPredicate})
			}
