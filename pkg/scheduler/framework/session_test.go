...
func TestUpdateQueueStatusStopsOnQueueHierarchyCycle(t *testing.T) {
	queueA := api.QueueID("queue-a")
	queueB := api.QueueID("queue-b")
	queues := map[api.QueueID]*api.QueueInfo{
		queueA: {UID: queueA, Name: string(queueA), Parent: queueB},
		queueB: {UID: queueB, Name: string(queueB), Parent: queueA},
	}
	...
	case <-time.After(time.Second):
		t.Fatal("updateQueueStatus did not stop for a cyclic queue hierarchy")
	}
}
