...
func (ssn *Session) Pipeline(task *api.TaskInfo, hostname string) error {
	...
	node, found := ssn.Nodes[hostname]
	if !found {
		klog.Errorf("Failed to find Node <%s> in Session <%s> index when pipeline.", hostname, ssn.UID)
...
func (ssn *Session) Allocate(task *api.TaskInfo, nodeInfo *api.NodeInfo) (err error) {
	...
	job, found := ssn.Jobs[task.Job]
	if !found {
		klog.Errorf("Failed to find Job <%s> in Session <%s> index when binding.", task.Job, ssn.UID)
	...
	node, found := ssn.Nodes[hostname]
	if !found {
		klog.Errorf("Failed to find Node <%s> in Session <%s> index when binding.", hostname, ssn.UID)
		return fmt.Errorf("failed to find node %s", hostname)
	}
	...
	if ssn.JobReady(job) {
		for _, task := range job.TaskStatusIndex[api.Allocated] {
			if err := ssn.dispatch(task); err != nil {
				klog.Errorf("Failed to dispatch task <%v/%v>: %v", task.Namespace, task.Name, err)
				return err
			}
		}
	} else {
...
func (ssn *Session) Evict(reclaimee *api.TaskInfo, reason string) error {
	...
	if !found {
		klog.Errorf("Failed to find Job <%s> in Session <%s> index when evicting.", reclaimee.Job, ssn.UID)
