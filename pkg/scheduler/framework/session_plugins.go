...
// AddVictimsFromCandidatesFns add VictimsFromCandidatesFns function
func (ssn *Session) AddVictimsFromCandidatesFns(name string, fns []api.VictimsFromCandidatesFn) {
	ssn.victimsFns[name] = fns
}
...
// Victims returns the victims selected
func (ssn *Session) Victims(tasks []*api.TaskInfo) map[*api.TaskInfo]bool {
	...
	for _, tier := range ssn.Tiers {
		for _, plugin := range tier.Plugins {
			...
			fns, found := ssn.victimsFns[plugin.Name]
			if !found {
				continue
			}
			for _, fn := range fns {
