...
func TestGenericPredicate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "parameter number match but type mismatch shoulde be also true",
			expect:    nil,
			parameter: []interface{}{1, 2},
		},
		{
			name:      "parameters are legeal but failed the first predication",
	...
	for _, s := range tests {
		err := sess.GenericPredicateFn(s.parameter...)
