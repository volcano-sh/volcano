...
func ImageLocalityPriorityMap(pod *v1.Pod, numOfNodes int, nodeInfo *schedulercache.NodeInfo) (schedulerapi.HostPriority, error) {
	node := nodeInfo.Node()
	if node == nil {
	...
	var score int
	score = calculatePriority(sumImageScores(nodeInfo, pod.Spec.Containers, numOfNodes))
