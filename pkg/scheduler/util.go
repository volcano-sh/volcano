...
func unmarshalSchedulerConf(confStr string) ([]framework.Action, []conf.Tier, []conf.Configuration, conf.Metrics, error) {
	var actions []framework.Action

	schedulerConf := &conf.SchedulerConfiguration{}

	if err := yaml.Unmarshal([]byte(confStr), schedulerConf); err != nil {
		return nil, nil, nil, conf.Metrics{}, err
	}
	...
	for i, tier := range schedulerConf.Tiers {
		...
		if hdrf && proportion {
			return nil, nil, nil, conf.Metrics{}, fmt.Errorf("proportion and drf with hierarchy enabled conflicts")
		}
	}
	...
	for _, actionName := range actionNames {
		if action, found := framework.GetAction(strings.TrimSpace(actionName)); found {
			...
			return nil, nil, nil, conf.Metrics{}, fmt.Errorf("failed to find Action %s, ignore it", actionName)
		}
	}
