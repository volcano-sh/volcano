...
// the whole logic of reclaim is as follows:
...
func (ra *Action) reclaimResources(ssn *framework.Session, queues *util.PriorityQueue, preemptorsMap map[api.QueueID]*util.PriorityQueue, preemptorTasks map[api.JobID]*util.PriorityQueue) {
	for !queues.Empty() {
		...
		assigned := ra.reclaimFromNode(ssn, task, job)
		if assigned {
			jobs.Push(job)
		}
		queues.Push(queue)
...
func (ra *Action) reclaimFromNode(ssn *framework.Session, task *api.TaskInfo, job *api.JobInfo) bool {
	assigned := false
	...
	for _, node := range totalNodes {
		...
		ra.collectAndProcessVitimTasks(ssn, node, task, reclaimees, &assigned)
...
func (ra *Action) collectAndProcessVitimTasks(ssn *framework.Session, node *api.NodeInfo, task *api.TaskInfo, reclaimees []*api.TaskInfo, assigned *bool) {
