	...
	for {
		...
		for {
			...
			for {
				...
				taskPipelinedInJob = taskPipelinedInJob || ra.reclaimForTask(ssn, stmt, task, job)
			}
...
func (ra *Action) reclaimForTask(ssn *framework.Session, stmt *framework.Statement, task *api.TaskInfo, job *api.JobInfo) bool {
