	...
	for _, job := range ssn.Jobs {
		if utilfeature.DefaultFeatureGate.Enabled(features.SchedulerPolicy) {
			...
			if schedulerPolicy != nil && !schedulerPolicy.HasAction(ra.Name()) {
				klog.Infof("%v's schedulerPolicy does not include the action %v.", job.Name, ra.Name())
				continue
			}

			if schedulerPolicy == nil && !ssn.HasAction(ra.Name()) {
				klog.Infof("Action %v is not defined in the global schedulingPolicy.", ra.Name())
				continue
			}
		}
