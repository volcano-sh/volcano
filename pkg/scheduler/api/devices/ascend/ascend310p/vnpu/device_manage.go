...
func (ns *NPUDevices) checkNPUResourceStable(pod *v1.Pod) error {

	iNum, iOK := ns.Idle[AscendNPUCore]
	nodeA, aOK := ns.Annotation[AscendNPUCore]
	if iOK != true || aOK != true {
		return fmt.Errorf("not has(or not same) %s", AscendNPUCore)
	}
	...
	if length > int(iNum/NPUHexKilo) {
		return fmt.Errorf("%s not stable:device-info is <%d> but k8s is <%d>", AscendNPUCore, length, int(iNum/NPUHexKilo))
	}
	return nil
}
...
func (ns *NPUDevices) checkNodeNum(pod *v1.Pod) error {
	...
	reqNPUNum, _ := ns.getAiCoreNumFromPod(pod)
	if !ok {
