...
func NewGPUDevices(name string, node *v1.Node) *GPUDevices {
	...
	if node.Status.Allocatable != nil {
		...
		if !gpuNumberExists || gpuNumberRes.Value() == 0 {
			klog.V(3).Infof("Node %s does not have allocatable volcano.sh/gpu-number resource or value is 0, returning nil", node.Name)
			return nil
		}
		...
		if !vgpuCoresExists || vgpuCoresRes.Value() == 0 {
			klog.V(3).Infof("Node %s does not have allocatable volcano.sh/vgpu-cores resource or value is 0, returning nil", node.Name)
			return nil
		}
		...
		if !vgpuMemoryExists || vgpuMemoryRes.Value() == 0 {
			klog.V(3).Infof("Node %s does not have allocatable volcano.sh/vgpu-memory resource or value is 0, returning nil", node.Name)
			return nil
		}
	} else {
