...
func (f MIGFactory) SubPod(gd *GPUDevice, mem uint, core uint, podUID string, devID string) {
    groupName, index, err := decodeMIGID(devID)
    if err != nil {
        klog.ErrorS(err, "Failed to sub pod")
        return
    }

    _, ok := gd.PodMap[podUID]
    if !ok {
        klog.Error("pod not exist in GPU pod map")
        return
    }
...
/*
The uuid for mig device is like this: GPU-0fc3eda5-e98b-a25b-5b0d-cf5c855d1448[group1,3]
  - The group1 is the name of the group; The "3" is the position in the group which
  - is resource count before this resource group + in-resource index - 1.
...
func pickFromGroup(group config.Geometry, usage config.MIGS, requestMemory uint) (bool, int, uint) {
    ...
    klog.V(4).Infoln("pick mig group but no suitalbe")
    return false, -1, 0
}
