...
func CheckUUID(annos map[string]string, id, useKey, noUseKey, deviceType string) bool {
	...
	if useUUID, ok := annos[useKey]; ok && strings.TrimSpace(useUUID) != "" {
		klog.V(5).Infof("check uuid for %s use uuid [%s], device id is %s", deviceType, useUUID, id)
		if !match(useUUID) {
			return false
		}
	}
	if noUseUUID, ok := annos[noUseKey]; ok && strings.TrimSpace(noUseUUID) != "" {
		klog.V(5).Infof("check uuid for %s no use uuid [%s], device id is %s", deviceType, noUseUUID, id)
		if match(noUseUUID) {
			return false
		}
	}
