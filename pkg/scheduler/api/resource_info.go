...
// but ignores any standard dimensions (MilliCPU, Memory) where rr has zero or less than minResource.
...
// @param req: Resource specifying which dimensions to compare.
// @return:
// (true, []string) if r exceeds rr in any filtered dimension, with the names of those resources;
// (false, []) if req is nil or no dimension exceeds.
