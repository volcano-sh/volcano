...
// CheckBind check expected bind result
func (test *TestCommonStruct) CheckHyperNodeBinpack(caseIndex int) error {
	binder := test.binder.(*util.FakeBinder)
	for i := 0; i < test.ExpectBindsNum; i++ {
		select {
		case <-binder.Channel:
	...
	expectStatistic := make([]int, 0, len(realHyperNode))
	for _, value := range realHyperNode {
		expectStatistic = append(expectStatistic, value)
	}
	sort.Ints(expectStatistic)

	if !slices.Equal(expectStatistic, test.ExpectBindpackStatistic) {
		return fmt.Errorf("case %d(%s) check bind: \nwant: %v\n got: %v ", caseIndex, test.Name, expectStatistic, test.ExpectBindpackStatistic)
	}
