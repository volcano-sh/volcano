		...
		},
		{
			...
			objects: []runtime.Object{
				&vcv1beta1.NamespaceQueue{
					...
					Spec:       vcv1beta1.NamespaceQueueSpec{Parent: "cluster/default"},
				},
			},
...
func TestNewDefaultAndRootQueuePreservesEmptyExistingAuthorization(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Empty(t, updated.Spec.AllowedNamespaces)

	for _, action := range client.Actions() {
		if action.GetVerb() == "update" && action.GetResource().Resource == "queues" {
			t.Fatalf("existing default Queue must not be updated")
		}
	}
}

func TestNewDefaultAndRootQueueDoesNotBackfillWhenNamespaceQueueDisabled(t *testing.T) {
