...
var DefaultAttachableVolumeQuantity int64 = 1000000000
...
func (sc *SchedulerCache) getPodCSIVolumes(pod *v1.Pod) (map[v1.ResourceName]int64, error) {
	...
	for _, vol := range pod.Spec.Volumes {
		...
		case vol.Ephemeral != nil:
			...
			isEphemeral = true
		default:
			// do not support Inline Volume now.
			continue
		}
		if pvcName == "" {
...
func (sc *SchedulerCache) getCSIDriverInfo(pvc *v1.PersistentVolumeClaim) string {
	...
	if csiSource == nil {
		// Don's support non-CSI volumes, will be support migrate pvc later
...
func (sc *SchedulerCache) syncNode(nodeName string) error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			err := sc.deleteNode(nodeName)
			if err != nil {
				klog.Errorf("Failed to delete node <%s> and remove from cache: %s", nodeName, err.Error())
				return err
			}
...
func (sc *SchedulerCache) updateNode(oldNode, newNode *v1.Node) error {
	if sc.Nodes[newNode.Name] != nil {
		...
		sc.removeNodeImageStates(newNode.Name)
...
// Assumes that lock is already acquired.
func (sc *SchedulerCache) deleteNode(node string) error {
	if _, ok := sc.Nodes[node]; !ok {
		return fmt.Errorf("node <%s> does not exist", node)
	}

	numaInfo := sc.Nodes[node].NumaInfo
	if numaInfo != nil {
...
func (sc *SchedulerCache) AddNode(obj interface{}) {
	...
	csiNode, err := sc.csiNodeInformer.Lister().Get(node.Name)
	if err == nil {
		sc.setCSIResourceOnNode(csiNode, node)
	} else if !errors.IsNotFound(err) {
		sc.resyncNode(node.Name)
		klog.Errorf("Failed to get csiNode %s: %v", node.Name, err)
	}
...
func (sc *SchedulerCache) UpdateNode(oldObj, newObj interface{}) {
	...
	csiNode, err := sc.csiNodeInformer.Lister().Get(newNode.Name)
	if err == nil {
		sc.setCSIResourceOnNode(csiNode, newNode)
	} else if !errors.IsNotFound(err) {
		klog.Errorf("Failed to get csiNode %s: %v", newNode.Name, err)
		sc.resyncNode(newNode.Name)
	}
...
func (sc *SchedulerCache) AddOrUpdateCSINode(obj interface{}) {
	...
	node, err := sc.nodeInformer.Lister().Get(csiNode.Name)
	if err != nil {
		klog.Errorf("Failed to get node %s, error: %v", csiNode.Name, err)
		sc.resyncNode(csiNode.Name)
		return
	}
...
func (sc *SchedulerCache) DeleteCSINode(obj interface{}) {
	...
	node, err := sc.nodeInformer.Lister().Get(csiNode.Name)
	if err != nil {
		klog.Errorf("Failed to get node %s, error: %v", node.Name, err)
		sc.resyncNode(csiNode.Name)
		return
	}
