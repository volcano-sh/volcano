...
func (sc *SchedulerCache) deleteTask(ti *schedulingapi.TaskInfo) error {
	...
	if len(ti.NodeName) != 0 {
		// We don't need to delete tasks that are already terminated, because
		// they will be deleted in an updateTask call and not being readded when
		// the taskStatus change happens from Releasing to -> Failed/Succeeded.
