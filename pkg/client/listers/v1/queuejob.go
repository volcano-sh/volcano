...
type QueueJobLister interface {
	...
	List(selector labels.Selector) (ret []*v1.QueueJob, err error)
...
// List lists all QueueJobs in the indexer.
func (s *queueJobLister) List(selector labels.Selector) (ret []*v1.QueueJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.QueueJob))
	})
	return ret, err
}
...
type QueueJobNamespaceLister interface {
	...
	Get(name string) (*v1.QueueJob, error)
}
...
// List lists all QueueJobs in the indexer for a given namespace.
func (s queueJobNamespaceLister) List(selector labels.Selector) (ret []*v1.QueueJob, err error) {
...
// Get retrieves the QueueJob from the indexer for a given namespace and name.
func (s queueJobNamespaceLister) Get(name string) (*v1.QueueJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("queueJob"), name)
	}
	return obj.(*v1.QueueJob), nil
}
