...
func Execute(ssn *framework.Session, jobs map[api.JobID]*api.JobInfo) (res *inspector.ScheduleResult) {
	...
	if len(ssn.Jobs) != len(jobs) {
		...
		res = &inspector.ScheduleResult{
			Error: inspector.NewScheduleError(fmt.Errorf("some bad happend ssn:%v jobs:%v len(jobs):%v", ssn.UID, ssn.Jobs, len(jobs))),
		}
		return res
	}
	...
	if len(jobsMap) == 0 {
		return inspector.FailResult(fmt.Errorf("no jobs to allocate"))
	}
...
func allocateResources(ssn *framework.Session, queues *util.PriorityQueue, jobsMap map[api.QueueID]*util.PriorityQueue) (res *inspector.ScheduleResult) {
	...
	for {
		...
		if ssn.Overused(queue) {
			...
			res = &inspector.ScheduleResult{
				Error: inspector.NewScheduleError(e),
			}
			continue
		}
...
func allocateResourcesForTasks(ssn *framework.Session, subJob *api.SubJobInfo, tasks *util.PriorityQueue, job *api.JobInfo, queue *api.QueueInfo, allNodes []*api.NodeInfo, hyperNode string) (res *inspector.ScheduleResult) {
	...
	res = &inspector.ScheduleResult{
		Tasks: make(map[string]*inspector.TaskResult),
	}
	...
	for !tasks.Empty() {
		...
		if !ssn.Allocatable(queue, task) {
			...
			res.Tasks[task.Name] = &inspector.TaskResult{
				Error: inspector.NewScheduleError(err),
			}
			res.Error = inspector.NewScheduleError(err)
