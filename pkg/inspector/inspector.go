...
func Execute(ssn *framework.Session, jobs map[api.JobID]*api.JobInfo) (res *ScheduleResult) {
	...
	ssn.Jobs = jobs
	pickUpQueuesAndJobs(ssn, queues, jobsMap)
	if len(jobsMap) == 0 {
...
func pickUpQueuesAndJobs(ssn *framework.Session, queues *util.PriorityQueue, jobsMap map[api.QueueID]*util.PriorityQueue) {
...
func allocateResources(ssn *framework.Session, queues *util.PriorityQueue, jobsMap map[api.QueueID]*util.PriorityQueue) (res *ScheduleResult) {
	...
	for {
		...
		res = allocateResourcesForTasks(ssn, tasks, job, queue, allNodes, "")
...
func allocateResourcesForTasks(ssn *framework.Session, tasks *util.PriorityQueue, job *api.JobInfo, queue *api.QueueInfo, allNodes []*api.NodeInfo, hyperNode string) (res *ScheduleResult) {
